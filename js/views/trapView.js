import { getTrapCards } from "../services/trapCardService.js";
import { getCart } from "../components/cart.js";

let currentPage = 1;
const pageSize = 12;

export function loadTrapCards(offset = 0) {
    const $spinner = $("#TrapCards .loading-spinner");
    const $grid = $("#TrapCards .cards-grid");

    $grid.empty().addClass("hidden");
    $spinner.removeClass("hidden");

    getTrapCards({ num: pageSize, offset })
        .then(response => {
            ...
            updatePaginationButtons(currentPage, cards.length, pageSize, $("#TrapCards"));
        })
        .catch(error => {
            console.error("Error al cargar las cartas trampa:", error);
        })
        .always(() => {
...
export function setupTrapPagination() {
    const $section = $("#TrapCards");
